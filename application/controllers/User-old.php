...
class user extends CI_Controller {
	...
	public function update($id)
	{
		...
		$user = dbgetwhere("user", ['IDUser' => $id])->row_array();
		if ($this->form_validation->run() == False) {
			$data = [
				"title" => "Dashboard",
				"role" => $this->db->get("role")->result_array(),
			...
			$data = [
				...
				"Username" => post("Username"),
				"Email" => post("Email"),
				"Password" => (post("Password")) ? password_hash(post("Password"), PASSWORD_DEFAULT) : $user['Password'] ,
				"IDRole" => post("IDRole"),
				"IsAktifUser" => 1
			];
			dbupdate("user", $data, ['IDUser' => $id]);
			set_flashdata("msg", swalfire("Data User Berhasil Diubah", "success"));
			redirect(base_url("user/index"));
		}
	}
