...
<?php $i = 0; foreach(dbgetwhere("menu", ["level_menu" => 1 ,'is_active' => 1])->result_array() as $key) : ?>
    <?php if ($key['is_head'] == '0') : ?>
        ...
        <!-- <tr class="bg-warning text-dark" style=" font-weight: bold;"> -->
...
<script>
    $(document).ready(function(){
        ...
        function loadDataRole() {
            const role_access = JSON.parse('<?= $role_access_menu; ?>');
            for (let i = 0; i < role_access.length; i++) {
                // console.log(role_access[i].IDMenu)
                if (role_access[i].is_read == "1") {
                    $("#read"+role_access[i].IDMenu).prop("checked", true)
                    $("#input-read-"+role_access[i].IDMenu).val("1")
                } else {
                    $("#read"+role_access[i].IDMenu).prop("checked", false)
                    $("#input-read-"+role_access[i].IDMenu).val("0")
                }
                if (role_access[i].is_create == "1") {
                    $("#create"+role_access[i].IDMenu).prop("checked", true)
                    $("#input-create-"+role_access[i].IDMenu).val("1")
                } else {
                    $("#create"+role_access[i].IDMenu).prop("checked", false)
                    $("#input-create-"+role_access[i].IDMenu).val("0")
                }
                if (role_access[i].is_update == "1") {
                    $("#update"+role_access[i].IDMenu).prop("checked", true)
                    $("#input-update-"+role_access[i].IDMenu).val("1")
                } else {
                    $("#update"+role_access[i].IDMenu).prop("checked", false)
                    $("#input-update-"+role_access[i].IDMenu).val("0")
                }
                if (role_access[i].is_delete == "1") {
                    $("#delete"+role_access[i].IDMenu).prop("checked", true)
                    $("#input-delete-"+role_access[i].IDMenu).val("1")
                } else {
                    $("#delete"+role_access[i].IDMenu).prop("checked", false)
                    $("#input-delete-"+role_access[i].IDMenu).val("0")
                }
            }
